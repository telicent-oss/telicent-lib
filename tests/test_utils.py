...
class TestGenerateGroupID(unittest.TestCase):
    ...
    def test_clean_hostname_non_k8s(self):
        ...
        for hostname, expected in zip(host_data["hosts"], host_data["expected_hosts"], strict=True):
    ...
    def test_clean_hostname_k8s(self):
        for hostname, expected in zip(host_data["k8s_hosts"], host_data["k8s_expected_hosts"], strict=True):
            result = clean_hostname(hostname)
    ...
    def test_generate_group_id_with_predefined_hosts(self, mock_stack, mock_gethostname):
        ...
        for mock_hostname, expected_group_id in zip(mock_hostnames, expected_group_ids, strict=True):
