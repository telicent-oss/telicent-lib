                            ...
                            with self.tracer.start_as_current_span("process output"):
                                if output_data is not None:
                                    ...
                                    if isinstance(output_data, list):
                                        ...
                                        conf = Configurator()
                                        security_label_header = conf.get("SECURITY_LABEL_HEADER", "Security-Label")
                                        if conf.get("DISABLE_PERSISTENT_HEADERS", "0") != "1" and \
                                                not RecordUtils.has_header(output_data, security_label_header) and \
                                                RecordUtils.has_header(record, security_label_header):
                                            label_headers = RecordUtils.get_headers(record, security_label_header)
